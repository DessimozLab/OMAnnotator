...
process exonerate {
    ...
    script:
    """
    exonerate --exhaustive --model protein2dna:bestfit --ryo ">%qi%ti %tcb %tce %tcl %tS\n%tcs\n" --showalignment no --showvulgar no --softmasktarget 1 --query $queryFile --target $targetGenome --querytype protein --targettype dna --subopt 0 --forcegtag 1 > "result_hit.fasta"
